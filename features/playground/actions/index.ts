...
// Valid template values from Prisma schema
type ValidTemplate = 'REACT' | 'NEXTJS' | 'EXPRESS' | 'VUE' | 'HONO' | 'ANGULAR';

const isValidTemplate = (template: string): template is ValidTemplate => {
  return ['REACT', 'NEXTJS', 'EXPRESS', 'VUE', 'HONO', 'ANGULAR'].includes(template);
};
...
export const createPlayground = async (data: {
    ...
    if (!isValidTemplate(data.template)) {
        throw new Error(`Invalid template value: ${data.template}. Must be one of: REACT, NEXTJS, EXPRESS, VUE, HONO, ANGULAR`);
    }
    ...
    try {
        const playground = await db.playground.create({
            data:{
                ...
                template: data.template as ValidTemplate,
                userId:user?.id!
            }
        })
