...
interface TerminalProps {
  ...
  className?: string;
  theme?: "dark" | "light";
  // Remove webContainerInstance prop since we'll use the service
}
...
const TerminalComponent = forwardRef<TerminalRef, TerminalProps>(({ 
  ...
  const handleTerminalInput = useCallback(async (data: string) => {
    ...
    switch (data) {
      case '\r': // Enter
        await executeCommand(currentLine.current);
        break;
  ...
  useEffect(() => {
    // Only initialize terminal on client side to avoid SSR issues
  ...
  }, [connectToWebContainer, isConnected]);
